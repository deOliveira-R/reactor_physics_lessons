...
- **CS4c CODA — the homogeneous problem is posed from its own data (the fabricated carrier retires)** — ⏹ PLAN + ANCHORS DELIVERED 2026-09-08, PRE-carve (`scratch/_step6/test_architect_verification_plan_coda.md`; **`tests/homogeneous/test_coda_anchors.py` 38 rows LANDED green** — 36 p / 2 strict-xfail, pyright 0 — + the frozen `_fixtures/coda_precarve_operators.json`; 7-arm battery over 415 rows / 3.4 s). ⭐⭐ REPRODUCED the census's mixture-direct bit-identity **8 of 8**, and STRENGTHENED the not-consumed half (an ALIEN carrier — `V=3`, node 11.5, SPHERICAL, a different `bulk_space` — moves nothing). ⛔⛔ TWO census/ruling refutations: the diffusion `mesh is None` refusal is **unreachable** post-retirement (the `ndim` gate fires first), not mis-worded; and G2.1 is a **live catcher** (arm `volumes ×2` reds it, G1.6 not) whose reference object survives a one-line re-key. 6 open rulings (O-1…O-6). → **`L81`**
