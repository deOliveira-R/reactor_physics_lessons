...
* **Execution shape `[P]`: two commits.** 6.2c-i the metric doctrine (`space.py`/`metric.py`: the derived object, the guard, the resolution order, `_apply_axes_weights` retired into `FactoredMetric`; bit-identical on every axis-built space; P4 pair) — independent of the head. 6.2c-ii the axis-built head + the Parseval unification in ONE commit (§6b: an axis-built head with the hub still reading the continuum head makes the frame's admission refuse every windowed operand, so the head, the hub's read of `basis_space`, `truncated`, the Landing A gate's re-pose, `moment_space_on`'s refusal (H-6), the windowed pin's re-baseline and the 7 metric-blind prose sites land together). Then the step's exit. The step's exit (the 13-tree canonical gate, predicted from the 6.0 memo's arithmetic and reconciled per tree) runs after 6.2c lands. Docs: the prose census's MUST-FIX sites per item (19 densifier, 17 identity + 9 nominal, 10 Optional, 6 `space_on`, 8 reflect) — theory pages by archivist per item, docstrings by the main agent in the carve; ONE sphinx build per commit; ⚠ "S3" is a 3-way homonym — every banner says "the identity flip (structural `__eq__`)", never "S3".
