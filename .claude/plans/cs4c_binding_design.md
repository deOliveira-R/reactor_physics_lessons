...
**Execution order (supersedes the table): 6.1 → 6.2a → 6.2b → [test-architect] → 6.2c → 6.3 (+ G3.3 harness gate + the `ELEGANCE-DEBT[guard]` issue) → 6.4 → 6.5.** Docs: the prose census's MUST-FIX sites per item (19 densifier, 17 identity + 9 nominal, 10 Optional, 6 `space_on`, 8 reflect) — theory pages by archivist per item, docstrings by the main agent in the carve; ONE sphinx build per commit; ⚠ "S3" is a 3-way homonym — every banner says "the identity flip (structural `__eq__`)", never "S3".
