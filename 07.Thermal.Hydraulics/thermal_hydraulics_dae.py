"""Coupled thermal-hydraulics and fuel behaviour under LOCA conditions (DAE version).
...
DAE version: coolant pressure is carried as a state variable with a stiff
relaxation equation dp/dt = -K*(p - p_target), matching the structure of
MATLAB's DAE system (mass matrix with zero rows for pressure). This prevents
wild pressure swings during Jacobian numerical differencing and avoids NaN
from out-of-range pyXSteam calls during post-failure LOCA blowdown.
"""
...
from data.materials.h2o_properties import h2o_density, h2o_enthalpy, h2o_properties
...
def _initialize_th(
    ...
    # Layout: fuel_T (nz*nf), clad_T (nz*nc), cool_h (nz), cool_p (nz),
    #         clad_epsPeff (nz*nc), clad_epsP (nz*nc*3)
    n_ode = nz * nf + nz * nc + nz + nz + nz * nc + nz * nc * 3
    y0 = np.zeros(n_ode)
    y0[: nz * nf] = T0_cool            # fuel temperatures
    y0[nz * nf : nz * nf + nz * nc] = T0_cool  # clad temperatures
    idx_h = nz * nf + nz * nc
    y0[idx_h : idx_h + nz] = h0_cool   # coolant enthalpy
    idx_p = idx_h + nz
    y0[idx_p : idx_p + nz] = p0_cool   # coolant pressure (MPa)
...
def _unpack(y: np.ndarray, p: dict) -> dict:
    ...
    return dict(
        fuel_T=fuel_T, clad_T=clad_T, cool_h=cool_h, cool_p=cool_p,
...
def _compute_pressure(cool_h, cool_props, vel0, T0, p0, p_node1, p):
    ...
    rho_inlet = h2o_density(max(p_node1, 0.05), T0)
...
def _rhs(t: float, y: np.ndarray, p: dict) -> np.ndarray:
    """Right-hand side of the ODE system (DAE version with pressure in state)."""
    nz, nf, nc = p["nz"], p["nf"], p["nc"]
    dz0 = p["dz0"]
    ...
    cool_p = state["cool_p"]     # (nz,) -- pressure from state vector
    ...
    # Water properties using state-vector pressure (like MATLAB)
    cool_props = []
    Lsat_arr = []
    Vsat_arr = []
    ...
    # Compute algebraic target pressure for relaxation equation
    # Use node 1 pressure from state vector for inlet density (matches MATLAB)
    cool_p_target, cool_vel, cool_mdot, cool_Re = _compute_pressure(
        cool_h, cool_props, vel0, T0, p0, cool_p[0], p
    )

    # Pressure relaxation rate: dp/dt = -K * (p_state - p_target)
    # Large K makes pressure track the algebraic target closely
    K_relax = 1e4
    rate_cool_p = -K_relax * (cool_p - cool_p_target)

    # Velocity and Re from state-vector pressure (already used for properties)
    ...
    # Deformed gap geometry — replicate MATLAB's funRHS.m line 272-275:
    #   gap.r_ = (clad.r(1) + fuel.dz)/2
    #   gap.a_ = 2*pi * gap.r_ .* (clad.dz(1) + fuel.dz)/2
    # MATLAB uses clad.r(1) (scalar: inner-most clad radius at first axial node)
    # mixed with fuel.dz (vector: deformed fuel node height), producing a gap
    # "radius" of ~0.75 m instead of ~4.2 mm. This is a MATLAB quirk but must
    # be matched for parity.
    gap_r_mid = 0.5 * (clad_r[0, 0] + fuel_dz)    # (nz,)  ~0.75 m
    gap_a_bnd = 2 * np.pi * gap_r_mid * 0.5 * (clad_dz[0, 0] + fuel_dz)  # (nz,)
    ...
    return _pack_rates(rate_fuel_T, rate_clad_T, rate_cool_h, rate_cool_p, rate_epsPeff, rate_epsP)
...
def solve_thermal_hydraulics(params: THParams | None = None) -> THResult:
    ...
    def _record_snapshot(t_val, y_val):
        ...
        records["cool_p"].append(state["cool_p"].copy())
    ...
    if not np.isnan(clad_fail_time):
        ...
        # Chunked integration for post-failure
        y_cur2 = y_fail.copy()
