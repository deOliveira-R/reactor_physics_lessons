#!/usr/bin/env python3
"""Fuel behaviour under 1-D radial approximation -- self-contained student script.
...
Physics
-------
The fuel pellet and cladding are modelled as concentric cylinders separated
by a gas-filled gap.  Heat is generated volumetrically in the fuel by
fission, conducted radially outward through the fuel, across the gap (by
gas conduction), through the cladding, and removed at the outer cladding
surface (fixed temperature BC).

Thermo-mechanical coupling includes:
  - UO2 thermal expansion, irradiation swelling, and irradiation creep
  - Zircaloy thermal expansion, irradiation creep, and plasticity
  - Fission gas release and internal gas pressure evolution
  - Gap closure detection and contact mechanics

The ODE state vector contains temperatures, fission density, volumetric
swelling, creep strains, and plastic strains.  Stresses are solved
algebraically at each RHS evaluation via a linear system (equilibrium +
