#!/usr/bin/env python3
"""Coupled 0-D reactor kinetics + 1-D thermal hydraulics + fuel/clad mechanics -- self-contained student script.
...
Physics
-------
Point kinetics with 6 delayed-neutron precursor groups drives the power.
Doppler feedback (fuel temperature) and moderator temperature feedback
(coolant temperature) provide negative reactivity coefficients that
limit the power excursion.

The thermal-hydraulic model is the same as Module 07 (1-D axial coolant
with two-phase flow regimes, radial fuel/clad heat conduction, cladding
elastic+plastic mechanics).

The ODE state vector contains: power, 6 precursor densities, fuel and
clad temperatures, coolant enthalpy, and cladding plastic strains.
Stresses and pressure are computed algebraically at each RHS evaluation.
