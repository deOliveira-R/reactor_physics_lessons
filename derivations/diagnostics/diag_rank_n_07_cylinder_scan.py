"""Scan of rank-N normalization coefficients for cylinder.

Test whether changing α_n fixes the cylinder non-monotone behavior or not.
If none of the coefficients work → confirms H1 (need 3D cylinder observer-centered quadrature).
"""
...
def solve_with_alpha(R, n_bc_modes, alpha_fn, geometry=CYLINDER_1D,
                     n_angular=24, n_rho=24, n_surf_quad=24):
    radii = np.array([R])
    r_nodes, r_wts, panels = composite_gl_r(radii, 2, 5, dps=25)
    K_vol = build_volume_kernel(geometry, r_nodes, panels, radii, _SIG_T,
    ...
    for n_mode in range(1, n_bc_modes):
        P_n = compute_P_esc_mode(geometry, r_nodes, radii, _SIG_T, n_mode,
                                 n_angular=n_angular, dps=25)
        g_n = compute_G_bc_mode(geometry, r_nodes, radii, _SIG_T, n_mode,
...
def run_ladder(geometry, R, alphas_dict, N_values=[1, 2, 3, 5]):
    print(f"\n=== {geometry.kind} R={R} MFP ===")
    print(f"{'N':>3} " + "  ".join(f"{name:>16}" for name in alphas_dict))
    for N in N_values:
        row = [f"{N:>3}"]
        for name, alpha_fn in alphas_dict.items():
            k = solve_with_alpha(R, N, alpha_fn, geometry=geometry)
            err = abs(k - _K_INF) / _K_INF
            row.append(f"{err*100:+6.2f}% k={k:.3f}")
        print("  ".join(row))
...
if __name__ == "__main__":
    ...
    for R in [1.0, 2.0, 5.0, 10.0]:
        run_ladder(CYLINDER_1D, R, alphas)
