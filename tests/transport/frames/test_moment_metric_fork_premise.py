r"""Anchors for CS4c step 6 item 6.2c — the moment-metric FORK, and its ruling.

⭐ **RULED 2026-09-08 (R-6.2c-1, the user): Parseval ``G⁻¹`` / ``G⁺`` on the
ONE moment space — "the carrier's norm is the field's energy".**  Item 6.2c-ii
landed it: the head is axis-built (its measure enters the identity), the
carrier's cached moment space, the operator ends and the frame's faces all
read the frame's Parseval-dressed ``basis_space``, and the continuum head
survives only as the basis's own coefficient space (``basis.space``,
``from_L``) — structurally UNEQUAL to the dressed one.  F1 below records the
state the ruling produced; F2–F4 are the measurements it was taken against,
unchanged (they were the theorem, not the seam).

**The fork, stated once (as it stood before the ruling).**  Two moment-head
spaces existed in the tree and carried DIFFERENT metrics:
...
def _head_measure(space) -> np.ndarray | None:
    """The head's diagonal metric where the axis doctrine put it — its single axis's measure."""
...
def test_the_two_moment_heads_are_UNEQUAL_and_the_dressed_one_is_the_frames(label: str, L: int) -> None:
    r"""RE-POSED at item 6.2c-ii (ruling R-6.2c-1) — the seam is gone.

    The continuum head (the basis's own ``space``) and the frame's
    Parseval-dressed head (``basis_space``) share family, order and name and
    are two SPACES: both axis-built, the measure in the identity. The dressed
    one is generated by the frame (it can re-dress itself at another order),
    the continuum one by the basis. On a DENSE-Gram row the dressed head's
    axis carries no measure and the matrix pseudo-inverse is positioned on
    the space's derived metric object (item 6.2c-i).

    Reddens under: re-installing the metric-blind identity, or a producer
    handing back the continuum head where the dressed one is bound.
    """
    frame = HarmonicFrame.from_galerkin(_RULES[label]().angular_frame(L))
    continuum, dressed = frame.basis.space, frame.basis_space

    assert continuum != dressed and dressed != continuum, "the metric is the identity now"
    ...
    assert continuum.axes[0].generator is frame.basis
    assert dressed.axes[0].generator is frame
    assert continuum.inner_product_weights is None and dressed.inner_product_weights is None

    cont = _head_measure(continuum)
    assert cont is not None and continuum.metric is None
    if (label, L) in _DENSE_ROWS:
        assert frame.discrete_gram_structure is GramStructure.DENSE
        assert _head_measure(dressed) is None and dressed.metric is not None
    else:
        assert frame.discrete_gram_structure is GramStructure.DIAGONAL
        dres = _head_measure(dressed)
        assert dressed.metric is None and dres is not None
        assert not np.array_equal(np.asarray(cont), np.asarray(dres))
